...
const SubmenuCategory = () => {
  ...
  return subCategories ? (
    <div className="flex md-center flex-col mb-6">
      <h3 className="text-center md-2xl mb-3 md-6">{t.categories}</h3>
      <div className="flex justify-center flex-wrap">
        {subCategories?.map((subCategory) => (
          <div
            className="flex flex-col items-center py-2 md-4 px-2 sm:px-3 md:px-6 bg-palette-card shadow-lg rounded-lg mx-1 my-1 md:mx-3 w-[5rem] sm:w-auto flex-grow cursor-pointer"
            key={subCategory.title}
            onClick={() => onClickHandler(subCategory.title)}
          >
            <subCategory.icon style={{ fontSize }} />
            <h4 className="text-center text-[12px] md-base md:pt-3">
              {t[subCategory.title]}
            </h4>
          </div>
        ))}
